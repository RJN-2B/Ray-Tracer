//Roland Nguyen Raytracer - Graphic 3350
#include "common.h"
...
bool shadowFeeler(vec4 p0){
  ...
  Object::IntersectionValues shadow_inter;
  shadow_inter.t_w = std::numeric_limits< double >::infinity();

  vec4 ray = lightPosition - p0;
  double t_length = length(ray);
  ray = normalize(ray);
  ray.w = 0;


  for (int j = 0; j < sceneObjects.size(); j++) {
	  shadow_inter = sceneObjects[j]->intersect(p0, ray);
	  if (shadow_inter.t_w < t_length) {
		  inShadow = true;
	  }
  }
  return inShadow;
...
float clamp(float x, float upper, float lower) { //Clamping function
	return min(upper, max(x, lower));
}

vec4 refrRay(const vec4 &I, const vec4 &N, vec4 &colorRef, const float &n_1, const float &n_2, Object *int_object, int depth) {
	float n1 = n_1;
	float n2 = n_2;
	float nDotI = dot(I, N);
	vec4 n = N;
	...
	float n_12 = n1 / n2;
	float thingInSqrt = 1 - n_12 * n_12 * (1 - pow(nDotI,2)); 
	if (thingInSqrt < 0) { //If Imaginary, reflect
		vec4 refl = normalize(reflect(I, N));
		return refl;
	}
	else {
		vec4 refractedRay = n_12 * I + (n_12 * nDotI - sqrtf(thingInSqrt)) * n;
		return refractedRay;
	}
}


float refrFres(const vec4 &I, vec4 N, const vec4 &T, float &n_1, float &n_2) {
	float n1 = n_1;
	float n2 = n_2;
	float nDotI = dot(I, N);
	...
	nDotI = dot(I, N);
	float nDotiTr = dot(T, N);
	float RP = pow(((n1 * nDotI) - (n2 * nDotiTr)) / ((n1 * nDotI) + (n2 * nDotiTr)), 2);
	float RS = pow(((n2 * nDotI) - (n1 * nDotiTr)) / ((n1 * nDotiTr) + (n2 * nDotI)), 2);
	float Kr = (RS + RP) / 2.0;
	return Kr;
}
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth){
  ...
  if(depth > maxDepth){ return color; } //This is background color being returned
  ...
  for (int j = 0; j < sceneObjects.size(); j++) {
	  Object::IntersectionValues intersect = sceneObjects[j]->intersect(p0, E);
	  intersect.ID_ = j;
	  if (intersect.t_w < min_intersect.t_w) {
  ...
  if (min_intersect.t_w != std::numeric_limits< double >::infinity()) {
	 ...
	 // double D = dot(normalize(lightPosition - min_intersect.P_w), min_intersect.N_w); //Diffuse component
	  vec4 L = normalize(lightPosition - min_intersect.P_w); 
	  L.w = 0;
	  vec4 R = normalize(reflect(L, min_intersect.N_w)); //REMOVE THE NEGATIVE
	  //R.w = 0.0;

	  
	  vec4 V = E;  //Normalize??  No, E Already unit.
	  V.w = 0.0;
	  ...
	  if (shadowFeeler(min_intersect.P_w) == false) {
		  color4 material_ambient(int_object->shadingValues.color.x*int_object->shadingValues.Ka,
			  int_object->shadingValues.color.y*int_object->shadingValues.Ka,
			  int_object->shadingValues.color.z*int_object->shadingValues.Ka, 1.0);
		  color4 material_diffuse(int_object->shadingValues.color.x*int_object->shadingValues.Kd,
			  int_object->shadingValues.color.y*int_object->shadingValues.Kd,
			  int_object->shadingValues.color.z*int_object->shadingValues.Kd, 1.0);
		  color4 material_specular(int_object->shadingValues.Ks,
			  int_object->shadingValues.Ks,
			  int_object->shadingValues.Ks, 1.0);
		  ...
		  float  material_shininess = int_object->shadingValues.Kn;



		  float Ks = pow(max(dot(V, R), 0.0), material_shininess);
		  specular = Ks * specular_product;

		  float Kd = fmax(dot(L, min_intersect.N_w), 0.0);
		  ...
		  if (int_object->shadingValues.Ks > 0) { //Then non-zero specular reflectivity
				  ...
				  vec4 ref = normalize(reflect(E, min_intersect.N_w));
				  //Now trace the ray using function: vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth)
				  vec4 colorRefl = castRay(min_intersect.P_w, ref, NULL, depth + 1);
				
				  colorRefl.w = 1; //I dunno if needed

				  specular = int_object->shadingValues.Ks * colorRefl;

		  }
		  vec4 colorRefr = vec4(0,0,0,1);
		  vec4 colorRefrRefl = vec4(0, 0, 0, 1);
		  if (int_object->shadingValues.Kt > 0) { //Then non-zero transmittance (Kt)
			  //Recompute Ks and Kt for glass sphere???
				  //First find refracted direction.
				  float n1 = 1;
				  float n2 = int_object->shadingValues.Kr;
				  vec4 refr;
				  

				  vec4 refl = normalize(reflect(E, min_intersect.N_w));
				  vec4 colorRefl = castRay(min_intersect.P_w, refl, NULL, depth + 1);


				  //Then TraceRay with refractedColor
				  vec4 T = normalize(refrRay(E, min_intersect.N_w, refl, n1, n2, int_object, depth + 1));
				  float kr = refrFres(E, min_intersect.N_w, T, n1, n2);

				  colorRefr = castRay(min_intersect.P_w, T, int_object, depth + 1);


				  float kt = 1 - kr;

				  colorRefrRefl = (colorRefr * kt) + (colorRefl * kr);
				  

		  }

		  color += ambient + specular + diffuse + colorRefrRefl;

		  color.w = 1.0; //w is the alpha term, make it 1
...
void initUnitSphere(){
  ...
  
  {
	  ...
	  _shadingValues.Kr = 0.0;
	  sceneObjects[sceneObjects.size() - 1]->setShadingValues(_shadingValues);
	  sceneObjects[sceneObjects.size() - 1]->setModelView(mat4(Translate(1.0, 0.0, 0.0) * Scale(0.5, 0.5, 0.5)));
  }
...
void initUnitSquare(){
  ...
  { //Back Wall
    ...
    Object::ShadingValues _shadingValues;
    //_shadingValues.color = vec4(1.0,1.0,1.0,1.0);
	_shadingValues.color = vec4(1.0, 0.0, 0.0, 1.0); //I added
    _shadingValues.Ka = 0.0;
    _shadingValues.Kd = 1.0;
    _shadingValues.Ks = 0.0;
    _shadingValues.Kn = 16.0;
